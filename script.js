...
(function(){
  ...
  // When reduce is set, show final state immediately.
  ...
  // Re-arm on scroll re-entry (v2 spec animation standard).
  ...
  // -- Reveal-on-scroll for sections (one-time entrance, must never leave content hidden) --
  ...
  // -- Animated counter for the stat number, re-runs on every scroll re-entry --
  // $600: high end of one emergency plumbing service/repair call in York County SC
  var STAT_TARGET = 600;
  var countRun = 0;
  var statMainEl = document.getElementById('statMain');
  var statCentsEl = document.getElementById('statCents');

  function showStatFinal(){
    countRun++;
    if (statMainEl) statMainEl.textContent = STAT_TARGET;
    if (statCentsEl) statCentsEl.textContent = '';
  }
  ...
  function runCount(){
    ...
    function step(ts){
      ...
      if (statMainEl) statMainEl.textContent = val;
  ...
  // Reduced motion from first paint: SMS thread and stat fully shown immediately
  if (reducedMotion()){ showThreadFinal(); showStatFinal(); }

  // A1: Hide sticky CTA bar when the real CTA panel is in view
